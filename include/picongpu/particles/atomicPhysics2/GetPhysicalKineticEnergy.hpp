    ...
    namespace particles
    {
        namespace atomicPhysics2
        {
            struct GetRealKineticEnergy
            {
                ...
                HDINLINE static float_X KineticEnergy(T_Particle const& particle) const
                {
                    constexpr float_X eV = static_cast<float_X>(
                        picongpu::UNIT_MASS * c_SI * c_SI * picongpu::UNITCONV_Joule_to_keV * 1e3);

                    m = attribute::getMass(1.0_X, particle) // unit: internal, not scaled
                    ...
                    // u_l^2 / (u_t^2*c_SI) = 1
                    float_X energy = (math::sqrt(m * m + momentumSquared) - m); // unit: u_m * c_SI^2
