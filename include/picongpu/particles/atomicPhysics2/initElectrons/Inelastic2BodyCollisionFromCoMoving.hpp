...
namespace picongpu::particles::atomicPhysics2::initElectrons
{
    ...
    public:
            ...
            T_RngGeneratorFloat& rngGenerator)
        {
            ...
            if(deltaEnergy <= 0._X)
            {
                if constexpr(picongpu::atomicPhysics2::debug::initIonizationElectrons::
            ...
            constexpr float_64 mc2_Ion = static_cast<float_64>(massIon) * UNIT_MASS
            ...
            // UNIT_MASS * UNIT_LENGTH / UNIT_TIME, not weighted
            constexpr float_64 mc_Electron
                = massElectron * picongpu::SI::SPEED_OF_LIGHT_SI / (UNIT_LENGTH / UNIT_TIME);
            ...
            // kg * m^2/s^2 * keV/J * 1e3 = J/J * eV = eV
            constexpr float_64 mc2_Electron = static_cast<float_64>(massElectron) * UNIT_MASS
                * pmacc::math::cPow(picongpu::SI::SPEED_OF_LIGHT_SI, 2u) * picongpu::UNITCONV_Joule_to_keV * 1.e3;
            ...
            MatrixVector momentumStarIon_LabSystem = momentumStarElectron_LabSystem /* ion = - electron */ * -1.
                + lorentzBoost.m_beta * (lorentzBoost.m_gamma * gammaStarIon_IonSystem * mcIon);
            // UNIT_MASS * UNIT_LENGTH / UNIT_TIME, weighted
            momentumStarElectron_LabSystem = momentumStarElectron_LabSystem
                + lorentzBoost.m_beta * (lorentzBoost.m_gamma * gammaStarElectron_IonSystem * mcElectron);
