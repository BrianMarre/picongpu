...
#include "picongpu/particles/atomicPhysics2/rateCalculation/BinomialCoefficient.hpp"
...
namespace picongpu::particles::atomicPhysics2::rateCalculation
{
    ...
    HDINLINE float_64 multiplicityConfigNumber(T_ConfigNumberStorage const configNumber)
    {
        ...
        for(uint8_t i = 0u; i < T_ConfigNumber::numberLevels; i++)
        {
            ...
            result *= picongpu::particles::atomicPhysics2::rateCalculation::binomialCoefficient(
    ...
    HDINLINE float_64 multiplicityBoundFreeTransition(T_LevelVector lowerState, T_LevelVector removedElectrons)
    {
        ...
        for(uint8_t i = 0u; i < T_LevelVector::dim; i++)
        {
            ...
            combinatorialFactor *= picongpu::particles::atomicPhysics2::rateCalculation::binomialCoefficient(
                lowerState[i],
                removedElectrons[i]);
        }
