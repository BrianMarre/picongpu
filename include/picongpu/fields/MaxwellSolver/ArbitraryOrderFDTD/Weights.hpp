                        ...
                        weights[0] = 4.0_X * T_neighbors
                            * powSpace::pow(
                                         (factorial(2 * T_neighbors)
                                          / float_X(
                                              powSpace::pow(2.0_X, 2u * T_neighbors)
                                              * powSpace::pow(factorial(T_neighbors), 2u))),
                                         2u);

                        // Compute all other values
                        for(uint32_t l = 1u; l < T_neighbors; ++l)
                        {
                            weights[l] = -1.0_X * powSpace::pow(float_X(l) - 0.5_X, 2u) * (T_neighbors - l)
                                / float_X(T_neighbors + l) / float_X(powSpace::pow(float_X(l) + 0.5_X, 2u))
                                * weights[l - 1];
                        }
                    }
